        ...
        <div class="flex justify-between h-16">
            <div class="flex items-center">
                ...
                <div class="hidden space-x-6 sm:-my-px sm:ms-8 sm:flex">
                    ...
                    <x-dropdown>
                        <x-slot name="trigger">
                            <button class="flex items-center space-x-1 px-3 py-2 text-sm font-medium text-gray-600 hover:text-red-600 transition-colors">
                        ...
                        <x-slot name="content" class="mt-2 py-2 bg-white rounded-lg shadow-xl border border-gray-100">
                            <x-dropdown-link :href="route('paket.index')" class="hover:bg-red-50 px-4 py-2">
                                <span class="{{ request()->routeIs('paket.index') ? 'text-red-600 font-semibold' : 'text-gray-700' }}">Paket</span>
                            </x-dropdown-link>
                            <x-dropdown-link :href="route('pelanggan.index')" class="hover:bg-red-50 px-4 py-2">
                                <span class="{{ request()->routeIs('pelanggan.index') ? 'text-red-600 font-semibold' : 'text-gray-700' }}">Pelanggan</span>
                            </x-dropdown-link>
                        </x-slot>
                    </x-dropdown>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-4">
                <!-- Notifications -->
                <a href="{{ route('notifikasi.index') }}" class="relative p-2 hover:bg-gray-100 rounded-full transition-colors">
                    <svg class="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                    </svg>
                    @if($notifikasiBaru > 0)
                        <span class="absolute top-0 right-0 bg-red-600 text-white text-xs px-2 py-1 rounded-full shadow">{{ $notifikasiBaru }}</span>
                    @endif
                </a>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="flex items-center space-x-1 group">
                            <div class="bg-gradient-to-r from-red-600 to-red-700 text-white px-4 py-2 rounded-full text-sm font-medium shadow-sm">
