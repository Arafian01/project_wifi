<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Form Tambah Pesan -->
            <div class="mb-4">
                <form action="{{ route('notifikasi.store') }}" method="POST" class="bg-white p-4 rounded-lg shadow-md flex gap-2">
                    @csrf
                    ...
                    <div class="bg-white p-4 rounded-lg shadow-md relative group">
                        ...
                        <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <div class="flex gap-2">
                                <form action="{{ route('notifikasi.index', $notifikasi->id) }}" method="GET" class="inline">
                                    <button type="submit" class="text-blue-500 hover:text-blue-700">
                                        ✏️
                                    </button>
                                </form>
                                <form action="{{ route('notifikasi.destroy', $notifikasi->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?');" class="inline">
                                    @csrf
                                    @method('DELETE')
