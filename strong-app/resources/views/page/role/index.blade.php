                ...
                <div class="p-6 text-gray-900 dark:text-gray-100 flex gap-5">
                    ...
                    <div class="w-full bg-gray-100 p-4 rounded-xl">
                        ...
                        <form action="{{ route('role.store') }}" method="post">
                            ...
                            <div class="mb-5">
                                <label for="nama"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Role</label>
                                    ...
                                    @foreach ($role as $key => $r)
                                        <tr
                                            ...
                                            <th scope="row"
                                                ...
                                                {{ $role->perPage() * ($role->currentPage() - 1) + $key + 1 }}
                                            </th>
                                            <td class="px-6 py-4">
                                                {{ $r->nama }}
                                            </td>
                                            <td class="px-6 py-4">
                                                <button type="button" data-id="{{ $r->id }}"
                                                    data-modal-target="sourceModal" data-nama="{{ $r->nama }}"
                                                    onclick="editSourceModal(this)"
                                                    class="bg-amber-500 hover:bg-amber-600 px-3 py-1 rounded-md text-xs text-white">
                                                    Edit
                                                </button>
                                                <button
                                                    onclick="return roleDelete('{{ $r->id }}','{{ $r->nama }}')"
...
<script>
    const editSourceModal = (button) => {
        ...
        document.getElementById('title_source').innerText = `Update Role ${nama}`;
    ...
    const roleDelete = async (id, nama) => {
            let tanya = confirm(`Apakah anda yakin untuk menghapus Role ${nama} ?`);
            if (tanya) {
                await axios.post(`/role/${id}`, {
                        '_method': 'DELETE',
